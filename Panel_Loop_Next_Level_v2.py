...
def auto_profile():
    ...
    if logical >= 16 and ram >= 24 and high_gpu:
        ...
        threads = min(12, max(1, logical - 1))
        ram_mb = min(11264, int(max(2048, (ram - 2) * 1024)))
    elif logical >= 16 and ram >= 24 and dedicated:
        tier = "High-End Competitive"
        threads = min(10, max(1, logical - 1))
        ram_mb = min(10240, int(max(2048, (ram - 2) * 1024)))
    ...
    return {
        ...
        "threads": threads,
        "ram_mb": ram_mb,
        "affinity": affinity_mask(logical, threads),
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.build_ui()
        self.refresh_cards()
        self.log("Panel Loop Next Level v2 started.")
    ...
    def build_ui(self):
        ...
        left = tk.Frame(body, bg="#07090d", width=355)
        left.pack(side="left", fill="y")

        tk.Label(left, text="⚡ EXECUTION PROTOCOLS", bg="#07090d", fg="white",
                 font=("Consolas", 16, "bold")).pack(anchor="w", pady=(0, 15))
    ...
    def animate_glow(self):
        ...
        color = "#00ffd5" if self.glow_state else "#00bfa8"
    ...
    def one_click(self):
        ...
        messagebox.showinfo("Panel Loop", "One Click complete.\nSet the same RAM/CPU values inside GameLoop Engine Settings.")
    ...
    def start_gameloop(self):
        ...
        try:
            ...
            return
        self.set_high_performance()
        self.log("Waiting 10 seconds for GameLoop processes...")
    ...
    def apply_network_stability(self):
        ...
        """
        run_ps(ps)
        self.log("Network/session stability preferences applied.")
